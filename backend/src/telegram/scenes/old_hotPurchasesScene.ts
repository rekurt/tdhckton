...
// const hotPurchasesScene = new Scenes.BaseScene<any>('HOT_PURCHASES_SCENE');

export function get(hotPurchasesScene) {
    ...
    hotPurchasesScene.enter((ctx) => {
        ...
        ctx.session.hotPurchases = ctx.session.hotPurchases.filter((p) => !p.isProcessed)
